...
// confirms which LG/TCL/Rakuten channels - every one of them, categorized or
// not - actually play from a Latin America vantage point. Run wherever this
// script happens to execute (a non-US IP), not from the US-based GitHub
// Actions runner. Existing checks (build.js's Amagi hostname pass) only
// detect what's geolocked OUT; this records the positive confirmation
// instead: channels seen live from here. Re-run by hand and commit the two
// output files whenever a fresh snapshot is wanted - it intentionally
// doesn't auto-update.
...
async function main() {
  ...
  const candidates = channels.filter((c) => c.provider === "lg" || c.provider === "tcl" || c.provider === "rakuten");
